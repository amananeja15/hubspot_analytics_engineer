/*
Question: Q3A – Maximum Possible Stay Per Listing
...
ranked as (
  select
    ...
    allowed_length,
    start_date,
    dateadd('day', allowed_length - 1, start_date) as allowed_end_date,
    row_number() over (
      partition by listing_id
      order by allowed_length desc, start_date asc
    ) as rn
  from valid
)
select
  ...
  allowed_end_date as best_end_date
from ranked
where rn = 1
